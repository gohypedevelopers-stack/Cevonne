...
DROP VIEW IF EXISTS public.g7_offer_source_view;
...
WITH collection_rows AS (
  SELECT
    ...
    d.applies_to_type,
    d.product_id AS discount_product_id,
    COALESCE(d.product_id, p.id) AS product_id,
    COALESCE(s.sku, p.slug) AS sku,
    d.collection_id,
    ...
    s.id AS variant_id,
    CASE WHEN s.id IS NOT NULL THEN 'ACTIVE' ELSE 'MISSING' END AS variant_status,
    COALESCE(i.quantity, 0) AS stock_available,
    'WEBSITE_INVENTORY' AS stock_source,
    COALESCE(i."updatedAt", now()) AS stock_checked_at,
    COALESCE(second_snap.quantity, 0) AS second_stock_available,
  ...
  INNER JOIN "Product" p
    ON p."collectionId" = d.collection_id
  LEFT JOIN LATERAL (
    SELECT s2.* 
    FROM "Shade" s2
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE g.sku = COALESCE(s.sku, p.slug)
    ORDER BY g.captured_at DESC
    LIMIT 1
  ) second_snap ON TRUE
  WHERE d.applies_to_type = 'SPECIFIC_COLLECTION'
    AND d.collection_id IS NOT NULL
),
all_products_rows AS (
  SELECT
    d.discount_id,
    d.code AS discount_code,
    d.discount_type,
  ...
  FROM public.cevonne_discounts d
  INNER JOIN "Product" p
    ON upper(d.applies_to_type) IN ('ALL_PRODUCTS', 'GLOBAL', 'ALL')
  LEFT JOIN "Collection" c
    ON c.id = p."collectionId"
  LEFT JOIN LATERAL (
    SELECT s2.*
    FROM "Shade" s2
    WHERE s2."productId" = p.id
...
single_product_rows AS (
  SELECT
    ...
    d.product_id AS discount_product_id,
    COALESCE(d.product_id, p.id) AS product_id,
    COALESCE(d.sku, v.sku, p.slug) AS sku,
    d.collection_id,
    d.status AS discount_status,
    d.starts_at AS discount_starts_at,
    d.ends_at AS discount_ends_at,
  ...
  LEFT JOIN LATERAL (
    SELECT g.quantity, g.source_type, g.source_name, g.evidence_url, g.captured_at
    FROM public.g7_inventory_snapshots g
    WHERE g.sku = COALESCE(d.sku, v.sku, p.slug)
    ORDER BY g.captured_at DESC
    LIMIT 1
  ) second_snap ON TRUE
  WHERE upper(d.applies_to_type) NOT IN ('SPECIFIC_COLLECTION', 'ALL_PRODUCTS', 'GLOBAL', 'ALL')
)
SELECT * FROM collection_rows
UNION ALL
SELECT * FROM all_products_rows
UNION ALL
SELECT * FROM single_product_rows;

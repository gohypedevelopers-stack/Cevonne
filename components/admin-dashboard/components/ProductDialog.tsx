...
type ProductDialogProduct = Product & {
  type?: string;
  description?: { headline?: string; body?: string } | string | null;
  pricing?: {
  ...
  experience?: {
    ...
    gallery?: Array<string | { id?: string; url?: string | null }> | null;
    badges?: Array<{ label?: string | null }> | string[] | string | null;
    benefits?: string[] | string | null;
    howToUse?: string[] | string | null;
    how_to_use?: string[] | string | null;
    claims?: string[] | string | null;
    disclaimer?: string | null;
    shipping?: string[] | string | null;
    returns?: string[] | string | null;
    ingredientsHighlight?: Array<{ name?: string; why?: string }> | null;
    ingredients_highlight?: Array<{ name?: string; why?: string }> | null;
    faqs?: Array<{ id?: string; q?: string; a?: string; question?: string; answer?: string }> | null;
    heroSection?: unknown;
  } | null;
  ingredients?: {
    supportingIngredients?: string[] | null;
    keyActives?: Array<{ name?: string; description?: string }> | null;
  } | null;
  badges?: Array<{ label?: string | null }> | string[] | string | null;
  shipping?: string[] | string | null;
  returns?: string[] | string | null;
...
function ProductFormBase({
  ...
  const { register, handleSubmit, setValue, reset, getValues, watch, control } = useForm<any>({
    defaultValues: {
      ...defaultValues,
  ...
  const shadeFields = shadeFieldRows as Array<{
    id?: string;
    name?: string;
    hexColor?: string;
    sku?: string;
  ...
  const ingredientFields = ingredientFieldRows as Array<{
    id?: string;
    name?: string;
    why?: string;
  }>;
  const faqFields = faqFieldRows as Array<{
    id?: string;
    q?: string;
    a?: string;
  }>;
  ...
  const handleImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const files = Array.from(input.files ?? []).slice(0, 6) as File[];
    setImages(files);
  };
  ...
  const handleDescriptionFile = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.currentTarget.files?.[0];
    if (!file) return;
...
type ProductDialogProps = Omit<ProductFormBaseProps, "layout" | "afterSubmit"> & {
  onClose?: (open: boolean) => void;
};

type ProductFormProps = Omit<ProductFormBaseProps, "layout">;

export function ProductDialog({ onClose = () => {}, ...props }: ProductDialogProps) {
  return (
    <ProductFormBase
      {...props}
      onClose={onClose}

...
import type { Product, ProductShade } from "@/types/product";

type ArShadesPanelProps = {
  shades?: ProductShade[];
  products?: Product[];
  loading?: boolean;
...
export function ArShadesPanel({ shades = [], products = [], loading, request, refresh }: ArShadesPanelProps) {
...
function ArShadeDialog({ open, onOpenChange, shade, products = [], request, refresh }: ArShadeDialogProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const uploadFile = async (file: File | null, field: "arAssetUrl" | "arPreviewUrl") => {
    if (!file) return;

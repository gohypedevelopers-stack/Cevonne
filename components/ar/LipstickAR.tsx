...
type LandmarkPoint3D = LandmarkPoint & {
  z: number;
};
...
function smoothPolyline(points: LandmarkPoint[], iterations = 1): LandmarkPoint[] {
...
function makePathFromRings(outerPts: LandmarkPoint[], innerPts?: LandmarkPoint[]) {
...
function computeBBox(points: LandmarkPoint[]): BoundingBox {
...
const rectFromPoints = (pts: LandmarkPoint[]): BoundingBox => computeBBox(pts);

const rectArea = (r: BoundingBox) => Math.max(0, r.w) * Math.max(0, r.h);



function rectPad(r: BoundingBox, pad: number): BoundingBox {
...
function rectIntersectArea(a: BoundingBox, b: BoundingBox): number {
...
function polygonArea(points: LandmarkPoint[]): number {
...
function computeCentroid(points: LandmarkPoint[]): LandmarkPoint {
...
function lipsArePresentHysteresis(outer_px: LandmarkPoint[], frameW: number, frameH: number, wasVisible: boolean) {
...
function smoothTemporal(prev: LandmarkPoint[] | null | undefined, curr: LandmarkPoint[], alpha: number): LandmarkPoint[] {
...
function stabilizeWithMotion(prev: LandmarkPoint[] | null | undefined, curr: LandmarkPoint[]): LandmarkPoint[] {
  ...
  const out: LandmarkPoint[] = new Array(n);
...
export default function VirtualTryOn() {
  ...
  const tintCanvasLeftRef = useRef<HTMLCanvasElement | null>(null);

  const tintCanvasRightRef = useRef<HTMLCanvasElement | null>(null);



  const faceMeshRef = useRef<MediaPipeController<MediaPipeFaceMeshResults> | null>(null);

  const handsRef = useRef<MediaPipeController<MediaPipeHandsResults> | null>(null);
  ...
  const latestResultsRef = useRef<MediaPipeFaceMeshResults | null>(null);

  const latestHandsRef = useRef<MediaPipeHandsResults | null>(null);
  ...
  const lastGoodLandmarksRef = useRef<LandmarkPoint[] | null>(null);

  const smoothedLandmarksRef = useRef<LandmarkPoint3D[] | null>(null);
  ...
  const shadeScrollerRef = useRef<HTMLDivElement | null>(null);

  const shadeButtonsRef = useRef<Record<number, HTMLButtonElement | null>>({});
  ...
  const occlAreaEmaRef = useRef<number | null>(null);

  const occlCentroidEmaRef = useRef<LandmarkPoint | null>(null);
  ...
  const prevOuterCssRef = useRef<LandmarkPoint[] | null>(null);

  const prevInnerCssRef = useRef<LandmarkPoint[] | null>(null);

  const prevOuterPxRef = useRef<LandmarkPoint[] | null>(null);

  const prevInnerPxRef = useRef<LandmarkPoint[] | null>(null);

  const edgeFeatherEmaRef = useRef<number | null>(null);
  ...
  function startProcessing() {
    ...
    if (!video || !frontCanvas || !backCanvas || !tintLeft || !tintRight || !mCanvas) return;
        ...
        if (!smoothedLandmarksRef.current) {
          ...
          for (let i = 0; i < raw.length; i++) {
            ...
            if (LIP_LANDMARK_INDICES.has(i)) {
              ...
              s.z += ((c.z ?? 0) - s.z) * (BASE_SMOOTHING * 0.5);
  ...
  function getLipPoints(landmarks: LandmarkPoint[], indices: number[], w: number, h: number): LandmarkPoint[] {
  ...
  function getLipPointsPx(landmarks: LandmarkPoint[], indices: number[], w: number, h: number): LandmarkPoint[] {
  ...
  function getHandBBoxesMirrored(
    handsResults: { multiHandLandmarks?: LandmarkPoint[][] } | null | undefined,
    w: number,
    h: number,
    padPx = 0
  ): BoundingBox[] {

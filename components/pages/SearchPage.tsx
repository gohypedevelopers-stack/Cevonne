...
type SearchProduct = Omit<Product, "description" | "media" | "pricing" | "tags"> & {
...
type SearchApiResponse = SearchProduct[] | { data?: SearchProduct[] };
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const filtered = catalog.filter((p) => {
                const description = typeof p.description === "object" && p.description ? p.description : null;
                const haystack = [
                    p.name,
                                    ...
                                    >
                                        <div className="relative aspect-square overflow-hidden rounded-xl bg-[var(--secondary-100)]">
                                            ...
                                            {product.media?.gallery?.[1] && (
                                                <img
                                                    src={resolveAsset(product.media.gallery[1].url ?? product.media.gallery[1].id ?? undefined)}

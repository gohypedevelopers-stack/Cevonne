...
const safeParse = (value: string): AddressRecord[] => {
  try {
    const parsed = JSON.parse(value) as unknown;
    return Array.isArray(parsed) ? (parsed as AddressRecord[]) : [];
  } catch (err) {
...
export const readAddresses = (): AddressRecord[] => {
  if (typeof window === "undefined") return [];
  const raw = window.localStorage.getItem(STORAGE_KEY);
  return raw ? safeParse(raw) : [];
};

export const writeAddresses = (addresses: AddressRecord[]) => {
  if (typeof window === "undefined") return;
  try {
...
export const addAddress = (address: Partial<AddressRecord> & { id?: string }): AddressRecord[] => {
  const existing = readAddresses();
  const entry: AddressRecord = {
    id: address?.id || newId(),
...
export const removeAddress = (id: string): AddressRecord[] => {
...
export const setDefaultAddress = (id: string): AddressRecord[] => {
  if (!id) return readAddresses();
